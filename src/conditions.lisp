(defpackage :structural-editing-mcp.conditions (:use :cl)
  (:export
...
(define-condition sexp-parse-error (structural-editing-error cl:parse-error)
  ...
  (:report
           (lambda
            ...
            (format
              stream
              "Parse error: ~A~@[ (token: ~S)~]"
              (or (slot-value condition 'message) "Invalid syntax")
              (parse-error-token condition))))
  (:documentation
                  "Signaled when parsing s-expressions encounters malformed tokens or unmatched delimiters."))
...
(define-condition invalid-path-error (structural-editing-error)
  ...
  (:documentation
                  "Signaled when navigating or updating an AST path that does not exist or is malformed."))
...
(define-condition workspace-error (structural-editing-error)
  ...
  (:documentation
                  "Signaled when a workspace operation fails (e.g. uninitialized workspace or missing file)."))