...
If EXACT is T, requires exact match; otherwise searches case-insensitively for substrings."
  (let
    ((results ' ())
     ...
     (start-node (if (and path (not (null path))) (get-node-at-path tree path) tree)))
    (when
          start-node
          (labels
        ((walk
              ...
              ((leaf node-path val)
               (let*
                  ((str (format-atom val)) (lower-str (string-downcase str)))
                  (when
                        (if exact (string= lower-query lower-str) (search lower-query lower-str))
  ...
  (let*
    ...
    (when
        ...
        ((walk
              ...
              (when
                    matched-p
                    (push (list :path (get-node-path node) :node node :bindings bindings) matches))
              (let
                ((children (get-node-children node)))
                (dolist (child children) (walk child))))))
        (walk start-node)))
    (nreverse matches)))

(defstruct (lint-finding (:constructor make-lint-finding)) rule
  path
  message
  severity
  suggested-fix)
...
(defun leaf-nil-p (node)
  ...
  (multiple-value-bind
    ...
    (and
         (eq tag :leaf)
         (or (null val) (and (symbolp val) (string-equal (symbol-name val) "NIL"))))))
    ...
    (and
         ...
         (or
          ...
          (and
             (symbolp val)
             (or
              (string-equal (symbol-name val) "T")
...
(defun check-if-progn-to-when (node path dialect)
  ...
  (let
    ...
    (when
        ...
        (when
              (and
               (member (get-node-tag then-node) ' (:paren :square))
               then-children
               (leaf-symbol-p (first then-children) "PROGN"))
              (let*
            ((body-nodes (rest then-children))
             (body-str
                        (if
                    body-nodes
            ...
            (make-lint-finding
              ...
              path
              :message
              "Prefer '(when ...)' over '(if ... (progn ...))' when there is no else branch."
              :severity
              :style
              :suggested-fix
              replacement)))))))
...
(defun check-if-nil-to-when (node path dialect)
  ...
  (let
    ...
    (when
        ...
        (unless
                (or
              (leaf-true-p then-node dialect)
              (and
                 (member (get-node-tag cond-node) ' (:paren :square))
            ...
            (make-lint-finding
              ...
              path
              :message
              "Prefer '(when <cond> <then>)' over '(if <cond> <then> nil)'."
              :severity
              :style
              :suggested-fix
              replacement)))))))
  ...
  (let
    ...
    (when
          (and
           ...
           (leaf-symbol-p (first children) "IF"))
          (let*
        ((test-node (second children)) (test-children (get-node-children test-node)))
        (when
              (and
               (member (get-node-tag test-node) ' (:paren :square))
               (= (length test-children) 2)
               (leaf-symbol-p (first test-children) "NOT"))
              (let*
            ((inner-cond (second test-children))
             (then-node (third children))
             (replacement
                           (format
                        nil
            ...
            (make-lint-finding
              ...
              path
              :message
              "Prefer '(unless <cond> <then>)' over '(if (not <cond>) <then>)'."
              :severity
              :style
              :suggested-fix
              replacement)))))))
    ...
    (when
        ...
        (when
            ...
            (make-lint-finding
              ...
              path
              :message
              "Invert negated condition: replace '(if (not <cond>) <then> <else>)' with '(if <cond> <else> <then>)'."
              :severity
              :style
              :suggested-fix
              replacement)))))))
...
(defun check-single-clause-cond (node path dialect)
  ...
  (let
    ...
    (when
          ...
          (let*
        ((clause (second children)) (clause-children (get-node-children clause)))
        (when
              (and
            ...
            (make-lint-finding
              ...
              path
              :message
              "'cond' with a single clause can be simplified to '(when <test> <body...>)'."
              :severity
              :style
              :suggested-fix
              replacement)))))))
...
(defun check-if-boolean-redundant (node path dialect)
  ...
  (let
    ...
    (when
        ...
        (make-lint-finding
          :rule
          :if-boolean-redundant
          :path
          path
          :message
          "Redundant conditional returning boolean; simplify '(if <cond> t nil)'."
          :severity
          :warning
          :suggested-fix
          replacement)))))
  ...
  (let
    ...
    (when
        ...
        (make-lint-finding
          ...
          path
          :message
          "Single-expression 'progn' is redundant."
          :severity
          :style
          :suggested-fix
          (sexp-to-string single-node))))))
...
(defun check-nested-let (node path dialect)
  ...
  (let
    ...
    (when
          ...
          (let*
        ((outer-bindings (second children)) (outer-body (nthcdr 2 children)))
        (when
              (and
               (= (length outer-body) 1)
               (member (get-node-tag outer-bindings) ' (:paren :square)))
              (let*
            ((inner-node (first outer-body))
             (inner-children (get-node-children inner-node)))
            (when
                  (and
                   (member (get-node-tag inner-node) ' (:paren :square))
                   (>= (length inner-children) 3)
                   (leaf-symbol-p (first inner-children) "LET"))
                  (let*
                ((inner-bindings (second inner-children))
                 (inner-body (nthcdr 2 inner-children)))
                (when
                      (member (get-node-tag inner-bindings) ' (:paren :square))
                      (let*
                    ((all-bindings
                                    (append (get-node-children outer-bindings) (get-node-children inner-bindings)))
                    ...
                    (make-lint-finding
                      ...
                      path
                      :message
                      "Cascaded nested 'let' forms can be combined into a single 'let*'."
                      :severity
                      :style
                      :suggested-fix
                      replacement)))))))))))
...
(defun check-equal-nil-to-null (node path dialect)
  ...
  (when
        (member dialect ' (:common-lisp :emacs-lisp nil))
        (let
      ((children (get-node-children node)))
      (when
            (and
             ...
             (= (length children) 3)
             (or
              (leaf-symbol-p (first children) "EQUAL")
              (leaf-symbol-p (first children) "EQ")
              (leaf-symbol-p (first children) "EQL")))
            (cond
          ((leaf-nil-p (third children))
           (let
              ((replacement (format nil "(null ~A)" (sexp-to-string (second children)))))
              (make-lint-finding
                :rule
                :equal-nil-to-null
                :path
                ...
                (format
                        nil
                        "Prefer '(null ~A)' over comparison with nil."
          ...
          ((leaf-nil-p (second children))
           (let
              ((replacement (format nil "(null ~A)" (sexp-to-string (third children)))))
              (make-lint-finding
                :rule
                :equal-nil-to-null
                :path
...
(defparameter *anti-pattern-rules*
    (list
        ...
        (list :id :invert-if-not :check #'check-invert-if-not :dialects nil)
        (list
          :id
          :single-clause-cond
          :check
...
(defun rule-matches-dialect-p (rule dialect)
  ...
  (let
    ((rule-dialects (getf rule :dialects)))
...
(defun rule-matches-filter-p (rule-id requested-rules)
  ...
  (if
      (null requested-rules)
      t
      (member
            (string-downcase (string rule-id))
...
(defun lint-node (node path &key (dialect :common-lisp) rules)
  ...
  (let
    ((findings ' ()))
    (dolist
            (r *anti-pattern-rules*)
            (let
        ((id (getf r :id)) (check-fn (getf r :check)))
        (when
              (and
               (rule-matches-dialect-p r dialect)
               (rule-matches-filter-p id rules)
               check-fn)
              (let
            ((finding (funcall check-fn node path dialect)))
...
Returns a list of LINT-FINDING instances."
  (let*
    ...
    (when
          ...
          (labels
        ((walk
                (node current-dialect)
                (let*
              ((tag (get-node-tag node))
               (node-path (get-node-path node))
               (effective-dialect
                  (cond
                    ((member tag ' (:common-lisp :clojure :scheme :emacs-lisp :fennel)) tag)
...
(defun format-lint-findings (findings)
  ...
  (if
      ...
      (loop
            ...
            ((stringp fix) (format s "   Suggested Fix: ~A~%" fix))))
            (format s "~%")))))

(defstruct (complexity-metrics (:constructor make-complexity-metrics)) name
  kind
  path
...
return (values is-def-p name-str kind-keyword)."
  (let
    ((children (get-node-children node)))
    (when
          (and (member (get-node-tag node) ' (:paren :square)) (>= (length children) 2))
          (let
        ((head (first children)))
        (multiple-value-bind
          (path tag val)
              ...
              (cond
                ...
                ((string= head-name "DEFINE")
                 (let
                    ...
                    (if
                        (member (get-node-tag name-child) ' (:paren :square))
                        (let
                        ((fn-head (first (get-node-children name-child))))
  ...
  (let
    ...
    (labels
      ((walk
            ...
            (when
                  (and (member (get-node-tag curr) ' (:paren :square)) children)
                  (let
                ((head (first children)))
                (multiple-value-bind
                  (p t-val val)
                  ...
                  (when
                        (symbolp val)
                        (let
                      ((name (string-upcase (symbol-name val))))
                      (cond
                        ((member
                                  name
                                  '
                                  ("IF" "WHEN" "UNLESS" "WHEN-NOT" "IF-NOT" "WHEN-LET" "IF-LET" "WHEN-FIRST")
                                  :test
                                  #'string=)
                         (incf complexity))
                        ((string= name "COND")
                         (dolist
                                  (clause (rest children))
                                  (let
                              ((c-children (get-node-children clause)))
                              (when
                                    (and (member (get-node-tag clause) ' (:paren :square)) c-children)
                                    (let
                                  ((test (first c-children)))
                                  (unless
                                          (or
                                        (leaf-true-p test dialect)
                        ...
                        ((member
                                  ...
                                  #'string=)
                         (dolist
                                  (clause (nthcdr 2 children))
                                  (let
                              ((c-children (get-node-children clause)))
                              (when
                                    (and (member (get-node-tag clause) ' (:paren :square)) c-children)
                                    (let
                                  ((selector (first c-children)))
                                  (unless
                                          (or (leaf-true-p selector dialect) (leaf-symbol-p selector "OTHERWISE"))
                                          (incf complexity)))))))
                        ((member name ' ("AND" "OR") :test #'string=)
                         (when (> (length children) 2) (incf complexity (- (length children) 2))))
                        ((member
                                  name
                                  '
                        ...
                        ((member name ' ("HANDLER-CASE" "RESTART-CASE") :test #'string=)
                         (dolist
                                  (clause (nthcdr 2 children))
                                  (when (member (get-node-tag clause) ' (:paren :square)) (incf complexity))))))))))
            (dolist (c children) (walk c)))))
      (walk node)
      complexity)))
...
(defun compute-nesting-depth (node &optional (current-depth 0))
  ...
  (let
    ...
    (if
        ...
        (let
        ((next-depth (1+ current-depth)) (max-child-depth (1+ current-depth)))
        (dolist
                (c children)
                (let
            ((d (compute-nesting-depth c next-depth)))
            (when (> d max-child-depth) (setf max-child-depth d))))
        max-child-depth)
        current-depth)))
...
(defun analyze-form-complexity (node &key path dialect)
  ...
  (multiple-value-bind
    ...
    (let*
      ((effective-name
                        (if is-def name (let ((tag (get-node-tag node))) (format nil ":~A" tag))))
    ...
    (cond
      ((eq tag :workspace)
       (loop
              ...
              (loop
                ...
                file-child
                in
                (get-node-children dialect-child)
                for
                f-idx
                from
                0
                ...
                (loop
                  ...
                  do
                  (push (cons form form-path) results)))))
      ((member tag ' (:common-lisp :clojure :scheme :emacs-lisp :fennel))
       (loop
              ...
              f-path
              =
              (or (get-node-path file-child) (append base-path (list f-idx)))
              do
              (loop
                for
                form
      ...
      ((eq tag :file)
       (loop
              ...
              form-path
              =
              (or (get-node-path form) (append base-path (list form-idx)))
              do
              (push (cons form form-path) results)))
      (t (push (cons node (or (get-node-path node) base-path)) results)))
    (nreverse results)))
...
Filters results to those meeting MIN-COMPLEXITY and MIN-DEPTH thresholds."
  (let*
    ((start-node (if (and path (not (null path))) (get-node-at-path tree path) tree))
     (forms-with-paths (if start-node (collect-top-level-forms start-node path) nil))
     (results ' ()))
    (dolist
            (pair forms-with-paths)
            (let*
        ((form-node (car pair))
         (form-path (cdr pair))
        ...
        (when
              (and
               (>= (complexity-metrics-cyclomatic-complexity metrics) (or min-complexity 1))
...
(defstruct (duplicate-group (:constructor make-duplicate-group)) code-snippet
  occurrence-count
  paths
  node-count
  depth
...
(defun canonicalize-subtree (node &key (exact t))
  ...
  (if
      exact
      (let*
      ((raw (sexp-to-string node))
       (cleaned (string-trim ' (#\  #\Newline #\Tab) raw)))
      cleaned)
      (labels
      ((anonymize
                   (curr is-head)
            ...
            ((leaf path val)
             (declare (ignore path))
             (if is-head curr (list :path nil :leaf '?_)))
            ((node path tag children)
             (list*
                     :path
                     path
                     tag
                     (loop for c in children for i from 0 collect (anonymize c (zerop i)))))
            (_ curr))))
...
(defun same-top-level-form-p (paths)
  ...
  (when
        ...
        (let
      ((first-parent
                      (if (<= (length (first paths)) 3) (first paths) (subseq (first paths) 0 3))))
      (every
             (lambda
                (p)
                (let
            ((parent (if (<= (length p) 3) p (subseq p 0 3))))
  ...
  (let*
    ...
    (when
        ...
        ((harvest
                   (curr curr-path)
                   (let
              ((tag (get-node-tag curr)) (children (get-node-children curr)))
              (when
                    (and
                     (member tag ' (:paren :square :curly))
                     children
                     (not
                       (member tag ' (:workspace :common-lisp :clojure :scheme :emacs-lisp :fennel))))
                    (let
                  ((node-cnt (count-ast-nodes curr)) (depth (compute-nesting-depth curr 0)))
                  (when
                      ...
                      (unless
                              (gethash fingerprint node-metadata)
                              (setf
                              (gethash fingerprint node-metadata)
    ...
    (let
      ...
      (maphash
               (lambda
                ...
                (>= (length entries) 2)
                (let*
              ((meta (gethash fingerprint node-metadata))
             ...
             (subsumed-p
                      ...
                      (lambda
                        ...
                        (and
                             ...
                             (every (lambda (cp) (some (lambda (op) (path-prefix-p op cp)) o-paths)) c-paths)))))
                      raw-candidates)))
            (unless subsumed-p (push cand filtered-candidates))))
        (let
          ((groups
                    (mapcar
                      (lambda
                    ...
                    ((paths (getf cand :paths))
                     ...
                     (single-line
                                   (substitute #\  #\Newline (string-trim ' (#\  #\Newline #\Tab) raw-str)))
                     (snippet
                               (if
...
(defstruct lexical-scope kind parent dialect enclosing-name (bindings nil))
    ...
    (loop
          ...
          (dolist
            ...
            (when
                  ...
                  (dolist
                      (var-node (rest spec-children))
                      (when (leaf-any-symbol-p var-node) (push (leaf-symbol-name var-node) ignored)))))))
                    ...
                    (when
                        ...
                        (cond
                          ((and
                                 (eq tag :paren)
                                 (or
                                  (leaf-any-symbol-p first-child)
                                  (and (eq (get-node-tag first-child) :paren) (get-node-children first-child))))
                           (if
                                (and (eq (get-node-tag first-child) :paren) (get-node-children first-child))
                                (let
                                ((sub (get-node-children first-child)))
                                (when (>= (length sub) 2) (collect (second sub))))
                                (collect first-child))
                           (when (>= (length children) 3) (collect (third children))))
                          (t (dolist (c children) (collect c))))))))
                ((eq tag :curly)
                 (let
                    ((children (get-node-children node)))
                    (loop
                          ...
                          do
                          (let
                        ((k-name (leaf-symbol-name k)))
                        (cond
                          ((and (equal k-name ":keys") (member (get-node-tag v) ' (:square :paren)))
                           (dolist (c (get-node-children v)) (collect c)))
                          ((and (equal k-name ":as") (leaf-any-symbol-p v)) (collect v))
                          ((leaf-any-symbol-p k) (collect k))))))))))))
      (if
          (member (get-node-tag params-node) ' (:paren :square))
        ...
        (cond
          ...
          (t
             (dolist
                    (clause children)
                    (cond
                ((leaf-any-symbol-p clause) (push (list :pattern clause :init nil) results))
                ((member (get-node-tag clause) ' (:paren :square))
                 (let
                    ((c-children (get-node-children clause)))
                    (push (list :pattern (first c-children) :init (second c-children)) results)))))))))
    (nreverse results)))
...
(defun check-and-register-binding (scope name path ignored-names findings)
  (let
    ...
    (when
          (and outer (not (ignored-variable-name-p name)))
          (push
            (make-binding-finding
          :kind
          :shadowed-variable
...
(defun check-unused-in-scope (scope dialect findings)
  (dolist
          (b (lexical-scope-bindings scope))
          (when
        ...
        (push
            ...
            nil
            :message
            (format nil "Variable '~A' defined in ~A is never used." name s-kind)
            :recommendation
            recomm)
              findings))))
  findings)
...
(defun walk-binding-tree (node scope dialect findings-acc)
  ...
  (when
        node
        (let
      ((tag (get-node-tag node)) (children (get-node-children node)))
      (cond
        ((eq tag :leaf)
         (when
                (leaf-any-symbol-p node)
        ...
        ((member
                  tag
                  '
                  (:workspace :file :common-lisp :clojure :scheme :emacs-lisp :fennel))
         (dolist
                  (c children)
        ...
        ((member tag ' (:paren :square))
         (when
              ...
              (cond
                ((member head-name ' ("quote" "'") :test #'string=) nil)
                ((and
                       (not (lisp-1-dialect-p dialect))
                       (member head-name ' ("function" "#'") :test #'string=))
                 nil)
                ((equal head-name "declare") nil)
                ((member
                          head-name
                          '
                          ("defun" "defmacro" "defmethod" "defn" "defn-" "define")
                          :test
                          #'string=)
                 (let*
                    ((name-child (second children))
                     (fn-name
                               (if (leaf-any-symbol-p name-child) (leaf-symbol-name name-child) "anonymous"))
                     (params-node nil)
                     (body-nodes nil))
                    (cond
                      ((and
                             (equal head-name "define")
                             (member (get-node-tag name-child) ' (:paren :square)))
                       (let
                       ...
                       (let
                          ...
                          (when
                                (and
                                 rem
                                 (or
                                  (stringp (third (parse-node (first rem))))
                                  (eq (get-node-tag (first rem)) :curly)))
                                (setf rem (rest rem)))
                          (if
                              (and rem (eq (get-node-tag (first rem)) :square))
                              (progn (setf params-node (first rem)) (setf body-nodes (rest rem)))
                              (setf body-nodes rem))))
                      (t (setf params-node (third children)) (setf body-nodes (cdddr children))))
                    (if
                        params-node
                        (multiple-value-bind
                        (ignored rem-body)
                        ...
                        (dolist
                              (form body-nodes)
                              (if
                            (member (get-node-tag form) ' (:paren :square))
                            (let*
                            ((f-children (get-node-children form))
                             (p-node (first f-children))
                             (b-nodes (rest f-children)))
                            (if
                                (and p-node (member (get-node-tag p-node) ' (:paren :square)))
                                (let*
                                ((fn-scope
                                            (make-lexical-scope
                                      :kind
                                ...
                                (dolist
                                        ...
                                        (setf
                                        findings-acc
                                        (check-and-register-binding fn-scope (car p) (cdr p) nil findings-acc)))
                                (dolist
                                        (b b-nodes)
                                        (setf findings-acc (walk-binding-tree b fn-scope dialect findings-acc)))
                                (setf findings-acc (check-unused-in-scope fn-scope dialect findings-acc)))
                ...
                ((member head-name ' ("lambda" "fn") :test #'string=)
                 (let*
                    ((rem (rest children)) (params-node nil) (body-nodes nil))
                    (when
                          (and rem (leaf-any-symbol-p (first rem)) (rest rem))
                          (setf rem (rest rem)))
                    (when rem (setf params-node (first rem)) (setf body-nodes (rest rem)))
                    (if
                        (and params-node (member (get-node-tag params-node) ' (:paren :square)))
                        (multiple-value-bind
                        (ignored rem-body)
                        (get-scope-declarations body-nodes dialect)
                        (let*
                ...
                ((and
                       (equal head-name "let")
                       (not (or (eq dialect :clojure) (eq dialect :fennel))))
                 (let*
                ...
                ((or
                      (member head-name ' ("let*" "letrec" "loop") :test #'string=)
                    ...
                    (multiple-value-bind
                      ...
                      (dolist
                          ...
                          ((step-scope
                                        (make-lexical-scope
                                :kind
                                (if (equal head-name "loop") :loop :let*)
                                :parent
                                curr-scope
                                :dialect
                                dialect))
                ...
                ((equal head-name "multiple-value-bind")
                 (let*
                    ...
                    (multiple-value-bind
                      ...
                      (let
                        ((mvb-scope
                                     (make-lexical-scope :kind :multiple-value-bind :parent scope :dialect dialect))
                         (bound (extract-param-bindings vars-node)))
                        (dolist
                                (p bound)
                                (setf
                                findings-acc
                                (check-and-register-binding mvb-scope (car p) (cdr p) ignored findings-acc)))
                        (dolist
                                (b rem-body)
                ...
                ((equal head-name "destructuring-bind")
                 (let*
                    ...
                    (multiple-value-bind
                      ...
                      (let
                        ((db-scope
                                    (make-lexical-scope :kind :destructuring-bind :parent scope :dialect dialect))
                         (bound (extract-param-bindings pat-node)))
                        (dolist
                                (p bound)
                                (setf
                                findings-acc
                                (check-and-register-binding db-scope (car p) (cdr p) ignored findings-acc)))
                        (dolist
                                (b rem-body)
                ...
                ((member head-name ' ("dolist" "dotimes") :test #'string=)
                 (let*
                    ...
                    (if
                        (and spec-node (member (get-node-tag spec-node) ' (:paren :square)))
                        (let*
                        ...
                        (multiple-value-bind
                          ...
                          (let
                            ((loop-scope
                                          (make-lexical-scope
                                  :kind
                                  (if (equal head-name "dolist") :dolist :dotimes)
                                  :parent
                                  scope
                                  :dialect
                                  dialect)))
                            (when
                                  (leaf-any-symbol-p var-node)
                                  (setf
                                    findings-acc
                                    (check-and-register-binding
                                  loop-scope
                ...
                ((member head-name ' ("flet" "labels") :test #'string=)
                 (let*
                    ...
                    (when
                          (and fns-node (member (get-node-tag fns-node) ' (:paren :square)))
                          (dolist
                              (f-def (get-node-children fns-node))
                              (when
                              (and f-def (member (get-node-tag f-def) ' (:paren :square)))
                              (let*
                            ((f-children (get-node-children f-def))
                             (fn-name-node (first f-children))
                             (fn-name
                                       (if (leaf-any-symbol-p fn-name-node) (leaf-symbol-name fn-name-node) "local-fn"))
...
(defun format-binding-report (findings)
  ...
  (if
      ...
      ((unused
                (remove-if-not
                         (lambda (f) (eq (binding-finding-kind f) :unused-variable))
                         findings))
       (shadowed
                  (remove-if-not
                         (lambda (f) (eq (binding-finding-kind f) :shadowed-variable))
                         findings)))
      (with-output-to-string
        (s)
        (format
        ...
        (when
              ...
              (loop
                ...
                (when
                  (binding-finding-recommendation f)
...
(defun priority-rank (p)
  ...
  (case p (:high 3) (:medium 2) (:low 1) (t 0)))
...
(defun parse-category-keyword (val)
  ...
  (let
    ((s (if (symbolp val) (symbol-name val) (string val))))
...
Returns a list of REFACTORING-SUGGESTION instances sorted by priority."
  (let*
    ((min-rank (priority-rank (parse-priority-keyword min-priority)))
     ...
     (suggestions ' ()))
    (when
          (or (null cat-keywords) (member :lint cat-keywords))
          (let
        ...
        (dolist
            ...
            ((rule (lint-finding-rule f))
             (p
                 (case
                   ...
                   :medium)
                      (t :low)))
             (tool (case rule (:redundant-progn "ast_remove") (t "ast_modify")))
             (plan
                    (if
                    (lint-finding-suggested-fix f)
            ...
            (when
                  ...
                  plan)
                    suggestions))))))
    (when
          (or (null cat-keywords) (member :complexity cat-keywords))
          (let
        ((complex-forms
                         (analyze-complexity
              tree
            ...
            ((cc (complexity-metrics-cyclomatic-complexity m))
             ...
             (p
                 (if
                    (or (>= cc 15) (>= depth 8))
                    :high
                    (if (or (>= cc 10) (>= depth 6)) :medium :low)))
    ...
    (when
          (or
          (null cat-keywords)
          (member :duplicate cat-keywords)
          (member :duplicates cat-keywords))
          (let
        ((duplicate-groups
                            (find-duplicate-subtrees tree :path path :min-nodes 3 :min-depth 2)))
        (dolist
                (g duplicate-groups)
                (let*
            ((savings
                       (* (duplicate-group-node-count g) (1- (duplicate-group-occurrence-count g))))
             (p (cond ((>= savings 20) :high) ((>= savings 8) :medium) (t :low)))
             (tool
                    (if
    ...
    (when
          (or
          (null cat-keywords)
          (member :binding cat-keywords)
          (member :bindings cat-keywords))
          (let
