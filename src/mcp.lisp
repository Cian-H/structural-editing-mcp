             ...
             (if (null children)
                 ...
                 (progn
                   ...
                   (loop for file-node in children
                         for idx from 0
                         for filepath = (structural-editing-mcp.workspace:get-filepath idx)
                         for form-count = (length (structural-editing-mcp.tree:get-node-children file-node))
                         do (format s "  [~A] :FILE (~A) — ~A top-level forms~%"
                                    idx (or filepath "unknown") form-count)))))
            ((eq tag :file)
             (let* ((file-idx (first path))
                    (filepath (and (numberp file-idx) (structural-editing-mcp.workspace:get-filepath file-idx))))
               (when filepath
                 (format s "File: ~A~%" filepath)))
       ...
       (let* ((parsed (structural-editing-mcp.parser:string-to-sexp wrapper-str))
         ...
         (if (and expr (structural-editing-mcp.tree:get-node-children expr))
             (let ((target-node (structural-editing-mcp.tree:get-node-at-path tree path)))
               (structural-editing-mcp.tree:update-node-at-path tree path
                 (lambda (node)
                   (declare (ignore node))
                   (match expr
                     ((node p tag children)
                      `(:path ,p ,tag ,@children ,target-node))))))
             (structural-editing-mcp.edit:wrap-node tree path :paren)))))))
...
Otherwise, PATH specifies the target location (parent is (butlast path), index is (lastcar path))."
  (let* ((has-explicit-index (not (null index)))
         (parent-path (if has-explicit-index
                          path
                          (if (null (cdr path))
                              path
                              (butlast path))))
         (target-idx (if has-explicit-index
                         index
                         (if (null (cdr path))
                             (length (structural-editing-mcp.tree:get-node-children
                                      (structural-editing-mcp.tree:get-node-at-path tree path)))
                             (lastcar path)))))
    (structural-editing-mcp.edit:insert-expression tree parent-path target-idx new-node-str)))
   ...
   (dict "name" "ast_modify"
         ...
         "inputSchema" (dict "type" "object"
                             "properties" (dict
                                           "path" (dict "type" "array"
                                                        "items" (dict "type" "integer")
                                                        "description" "Target AST path (e.g. [0, 2] to target form 2 in file 0).")
                                           "action" (dict "type" "string"
                                                          "enum" (list "insert" "overwrite" "wrap")
                                                          "description" "The modification action to perform.")
                                           "new_node" (dict "type" "string"
                                                            "description" "For insert/overwrite: the S-expression code string (e.g. '(defun foo () 42)'). For wrap: delimiter keyword (':paren', ':square', ':curly') or enclosing form string (e.g. '(when condition)').")
                                           "index" (dict "type" "integer"
                                                         "description" "Optional child index for insert. If omitted when inserting, uses the last element of path."))
   ...
   (dict "name" "ast_remove"
         ...
         "inputSchema" (dict "type" "object"
                             "properties" (dict
                                           "path" (dict "type" "array"
                                                        "items" (dict "type" "integer")
                                                        "description" "The AST path of the node to remove/unwrap/promote.")
                                           "action" (dict "type" "string"
                                                          "enum" (list "delete" "unwrap" "promote")
                                                          "description" "Removal action to perform."))
   ...
   (dict "name" "ast_relocate"
         ...
         "inputSchema" (dict "type" "object"
                             "properties" (dict
                                           ...
                                           "target_path" (dict "type" "array"
                                                               "items" (dict "type" "integer")
                                                               "description" "Target path for move/copy/swap/merge (e.g. [0, 2] places before index 2 in file 0).")
                                           "action" (dict "type" "string"
                                                          "enum" (list "move" "copy" "swap" "merge")
                                                          "description" "Relocation action to perform.")
                                           "index" (dict "type" "integer"
...
(defun handle-initialize (id params)
  ...
  (send-result
   id
   (dict "protocolVersion" "2024-11-05"
         "capabilities" (dict "tools" (make-hash-table))
         "serverInfo" (dict "name" "structural-editing-mcp"
                            "version" "0.1.0"))))
...
(defun handle-tools-call (id params)
  (let ((name (gethash "name" params))
    ...
    (handler-case
               ...
               (cond
                    ...
                    (cond
                      ((equal action "insert")
                       (setf structural-editing-mcp.workspace:*workspace-tree*
                             (perform-insert structural-editing-mcp.workspace:*workspace-tree* path new-node-str index)))
                 ...
                 ((equal name "ast_remove")
                  (let* ((path (to-list (gethash "path" args)))
                    ...
                    (cond
                      ((equal action "delete")
                       (setf structural-editing-mcp.workspace:*workspace-tree*
                             (structural-editing-mcp.edit:delete-node structural-editing-mcp.workspace:*workspace-tree* path)))
                      ((equal action "unwrap")
                       (setf structural-editing-mcp.workspace:*workspace-tree*
                             (structural-editing-mcp.edit:unwrap-node structural-editing-mcp.workspace:*workspace-tree* path)))
                 ...
                 ((equal name "ast_relocate")
                  (let* ((src (to-list (gethash "source_path" args)))
                    ...
                    (cond
                      ((equal action "move")
                       (multiple-value-bind (tgt-parent tgt-idx) (resolve-parent-and-index structural-editing-mcp.workspace:*workspace-tree* tgt index)
                         (setf structural-editing-mcp.workspace:*workspace-tree*
                               (structural-editing-mcp.edit:move-node structural-editing-mcp.workspace:*workspace-tree* src tgt-parent tgt-idx))))
                      ((equal action "copy")
                       (multiple-value-bind (tgt-parent tgt-idx) (resolve-parent-and-index structural-editing-mcp.workspace:*workspace-tree* tgt index)
                         (setf structural-editing-mcp.workspace:*workspace-tree*
      ...
      (error (e)
        (send-result id (dict "content" (list (dict "type" "text" "text" (format nil "Error: ~A" e)))
                              "isError" t))))))
...
(defun handle-message (msg)
  ...
  (let ((jsonrpc (gethash "jsonrpc" msg))
    ...
    (cond
      ...
      (id
       (send-error id -32601 (format nil "Method not found: ~A" method))))))
