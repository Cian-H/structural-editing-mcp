(defpackage :structural-editing-mcp.tree (:use :cl :trivia :alexandria)
  (:export
           :get-node-at-path
           :reindex-paths
           :update-node-at-path
           ...
           :leaf)
  (:documentation
                  "Core AST representations, patterns, accessors, and path navigation."))

(in-package :structural-editing-mcp.tree)

(declaim (optimize (speed 3) (safety 0) (debug 0)))

(defpattern node (path tag children) ` (list* :path ,path ,tag ,children))

(defpattern leaf (path val) ` (list :path ,path :leaf ,val))

(defpattern comment (path text) ` (list :path ,path :comment ,text))
...
(defun get-node-path (node)
  ...
  (match node ((or (node path _ _) (leaf path _) (comment path _)) path) (_ nil)))
...
(defun get-node-at-path (tree path)
  ...
  (loop
        ...
        children
        =
        (get-node-children current)
        for
        sub
        =
        (and children (>= idx 0) (nthcdr idx children))
        if
        sub
        do
        (setf current (car sub))
        else
        return
        nil
        finally
...
(defun reindex-paths (tree &optional (current-path ' ()))
  ...
  (match
    tree
    ((leaf _ val) (list :path current-path :leaf val))
    ((comment _ text) (list :path current-path :comment text))
    ((node _ tag children)
     (list*
             :path
             current-path
             ...
             (loop
              ...
              child-path
              =
              (append current-path (list idx))
              collect
              (reindex-paths child child-path))))
    (_ tree)))
             ...
             (let
                ...
                (list*
                       ...
                       (loop
                        ...
                        0
                        collect
                        (if (= i child-idx) (walk child (cdr p) (append curr-path (list i))) child)))))
            (_ elem)))))
    (walk tree path ' ())))