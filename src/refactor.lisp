(defpackage :structural-editing-mcp.refactor
  (:use :cl :structural-editing-mcp.tree :structural-editing-mcp.parser :alexandria)
...
(defun match-pattern (pattern target bindings)
  ...
  (cond
    ...
    ((and (eq (get-node-tag pattern) :leaf)
          (eq (get-node-tag target) :leaf))
     ...
     (let ((pchildren (get-node-children pattern))
       ...
       (if (= (length pchildren) (length tchildren))
           (loop for p in pchildren
                 ...
                 do (multiple-value-bind (success new-bindings) (match-pattern p t-child bindings)
                      (if success
...
(defun replace-pattern (tree pattern-str replacement-str)
  ...
  (let* ((pat-ast (first (get-node-children (string-to-sexp pattern-str))))
...
(defun extract-variable (tree target-path var-name)
  ...
  (when (or (null target-path) (null (cdr target-path)))
  ...
  (let* ((target-node (get-node-at-path tree target-path))
    ...
    (update-node-at-path tree parent-path
      (lambda (parent)
        (multiple-value-bind (p-path p-tag p-children) (parse-node parent)
          (declare (ignore p-path))
          (let ((new-children (loop for child in p-children
                                    for i from 0
                                    collect (if (= i child-idx)
                                                (list :path nil :leaf (intern (string-upcase var-name)))
                                                child))))
            (let ((new-parent `(:path nil ,p-tag ,@new-children)))
              (let ((let-ast (string-to-sexp (format nil "(let ((~A )))" var-name))))
                (let ((let-node (first (get-node-children let-ast))))
                  (let* ((bindings-list (second (get-node-children let-node)))
                         (first-binding (first (get-node-children bindings-list)))
                         (completed-binding `(:path nil :paren ,@(get-node-children first-binding) ,target-node))
                         (completed-bindings-list `(:path nil :paren ,completed-binding)))
                    `(:path nil :paren 
                            (:path nil :leaf let)
                            ,completed-bindings-list
                            ,new-parent)))))))))))
