...
(defun parse-string-escape (string index len out)
  "Parse an escape sequence in a string literal starting at INDEX (after the backslash).
Returns the updated index."
  (declare (type string string) (type fixnum index len) (type stream out))
  (if (>= index len)
    ...
    (let ((esc (char string index)))
      (cond
        ((char= esc #\n) (write-char #\newline out) (incf index))
        ((char= esc #\t) (write-char #\tab out) (incf index))
        ((char= esc #\r) (write-char #\return out) (incf index))
        ((char= esc #\b) (write-char (code-char 8) out) (incf index))
        ((char= esc #\f) (write-char (code-char 12) out) (incf index))
        ((char= esc #\0) (write-char (code-char 0) out) (incf index))
        ((char= esc #\\) (write-char #\\ out) (incf index))
        ((char= esc #\") (write-char #\" out) (incf index))
        ((char= esc #\') (write-char #\' out) (incf index))
        ((char= esc #\u)
          (cond
            ((and (< (1+ index) len) (char= (char string (1+ index)) #\{))
              (let
                  ((close-pos (position #\} string :start (+ index 2) :end (min len (+ index 10)))))
                (if
                    (and close-pos
                         (> close-pos (+ index 2))
                         (every
                  ...
                  (let
                      ((code (parse-integer string :start (+ index 2) :end close-pos :radix 16)))
                    (write-char (or (code-char code) #\?) out)
                    (setf index (1+ close-pos)))
                  (progn (write-char esc out) (incf index)))))
            ((and (<= (+ index 5) len)
                  (every
                    (lambda (c)
                      (digit-char-p c 16))
              ...
              (let
                  ((code (parse-integer string :start (1+ index) :end (+ index 5) :radix 16)))
                (write-char (or (code-char code) #\?) out)
                (incf index 5)))
            (t (write-char esc out) (incf index))))
        ((char= esc #\U)
          (if
              (and (<= (+ index 9) len)
                   (every
            ...
            (let
                ((code (parse-integer string :start (1+ index) :end (+ index 9) :radix 16)))
              (write-char (or (code-char code) #\?) out)
              (incf index 9))
            (progn (write-char esc out) (incf index))))
        ((char= esc #\x)
          (cond
            ((and (< (1+ index) len) (char= (char string (1+ index)) #\{))
              (let
                  ((close-pos (position #\} string :start (+ index 2) :end (min len (+ index 6)))))
                (if
                    (and close-pos
                         (> close-pos (+ index 2))
                         (every
            ...
            ((and (<= (+ index 3) len)
              ...
              (let
                  ((code (parse-integer string :start (1+ index) :end (+ index 3) :radix 16)))
                (write-char (or (code-char code) #\?) out)
                (incf index 3)))
            (t (write-char esc out) (incf index))))
        (t (write-char esc out) (incf index))))))
...
(defun read-string-literal
       (string index len &optional (delimiter #\") dialect)
  ...
  (let ((out (make-string-output-stream)))
    ...
    (loop while
          ...
          (cond
            ((char= ch #\\) (incf index)
                            (setf index (parse-string-escape string index len out)))
            ((char= ch delimiter) (incf index)
...
(defun parse-read-literal (tok predicate-fn)
  ...
  (let
      ((parsed (ignore-errors (let ((*read-eval* nil)) (read-from-string tok)))))
    (when (and parsed (funcall predicate-fn parsed)) parsed)))

(defun parse-character-token (string start end)
  "Parse character token starting with #\\."
  (let ((tok (subseq string start end)))
    (or (parse-read-literal tok #'characterp) (intern (string-upcase tok)))))
...
(defun parse-token (string start end)
  ...
  (let ((len (- end start)))
    ...
    (cond
      ...
      ((and (char= (char string start) (code-char 35))
            (eql (peek-char-ahead string start end) #\\))
...
(defun skip-atom-chars
       (string index len &optional (dialect *current-dialect*))
  "Advance and return INDEX past all characters that are neither whitespace nor delimiters."
  (declare (type string string) (type fixnum index len))
  (loop while
        (and (< index len)
...
(defun read-line-comment-token (string index len)
  ...
  (let ((start index))
    (incf index)
    (loop while
          (and (< index len) (not (char= (char string index) #\newline)))
          do
          (incf index))
    (when (and (< index len) (char= (char string index) #\newline)) (incf index))
    (values (list :comment (subseq string start index)) index)))

(defun step-block-comment-depth
       (string index len depth)
...
(defun read-block-comment-token
       (string index len)
  "Read block comment #|...|# starting at INDEX. Return (values token-entry next-index)."
  (let ((start index) (depth 1))
    (incf index 2)
    (loop while
          (and (< index len) (> depth 0))
          do
          (multiple-value-bind (new-depth inc)
...
(defun tokenize-next-token
       (string index len dialect)
  "Read the next token starting at INDEX in STRING. Return (values token next-index has-tok-p)."
  (let ((ch (char string index)))
    (cond
      ((whitespace-p ch dialect) (values nil (1+ index) nil))
      ((char= ch #\;)
        (multiple-value-bind (tok next)
                             (read-line-comment-token string index len)
          (values tok next t)))
      ((char= ch #\#)
        (multiple-value-bind (tok next)
                             (read-dispatch-macro-token string index len dialect)
          (if next
            (values tok next t)
            (multiple-value-bind (atok anext)
                                 (read-default-atom-token string index len dialect)
              (values atok anext t)))))
      ((and (eq dialect :fennel)
            (char= ch #\[)
            (eql (peek-char-ahead string index len) #\[))
        (let
            ((close-pos (search "]]" string :start2 (+ index 2))))
          (if close-pos
            ...
            (error 'sexp-parse-error
                   ...
                   (subseq string index)
                   :message
                   "Unterminated Fennel multiline string [[...]]"))))
      ((delimiter-char-token ch)
        (values (cons :delim (delimiter-char-token ch)) (1+ index) t))
      ((char= ch #\")
        (multiple-value-bind (tok next)
                             (read-string-literal string index len #\" dialect)
          (values tok next t)))
      (t
        (multiple-value-bind (tok next)
                             (read-default-atom-token string index len dialect)
...
(defun parse-collection
       (close-token tokens current-path child-index)
  "Parse sibling forms until CLOSE-TOKEN, returning (values children remaining-tokens)."
  (let
      ((remaining tokens) (children '()) (idx child-index))
    (loop
      (match remaining
             (nil
               (error 'sexp-parse-error
                      ...
                      close-token
                      :message
                      (format nil "Unexpected end of input: missing ~A" close-token)))
             ((list* (cons :delim (guard tok (eq tok close-token))) rest)
              (return (values (nreverse children) rest)))
             (_
               (let
                   ((child-path (append current-path (list idx))))
                 (multiple-value-bind (child after-child)
                                      (parse-single-form remaining child-path)
                   (push child children)
                   (incf idx)
  ...
  (match tokens
         ...
         ((list* (cons :delim (or :paren-close :square-close :curly-close)) _)
          (error 'sexp-parse-error
                 ...
                 (car tokens)
                 :message
                 (format nil "Unexpected closing delimiter: ~A" (car tokens))))
         ((list* (list :comment text) rest)
          (values (list :path path :comment text) rest))
         ((list* atom rest) (values (list :path path :leaf atom) rest))))

(defun string-to-sexp
       (string &key (dialect *current-dialect*))
  ...
  (let* ((*current-dialect* dialect))
    (multiple-value-bind (tokens starts ends)
      ...
      (if (null tokens)
        ...
        (let
            ((remaining tokens) (forms '()) (tok-spans '()) (idx 0) (tok-idx 0))
          (loop while
                remaining
                do
                (let ((start-tok tok-idx))
                  (multiple-value-bind (form rest)
                                       (parse-single-form remaining (list idx))
                    (let*
                        ((consumed (- (length remaining) (length rest)))
              ...
              ((slices
                 (loop for
                       ...
                       (if next-span
                         ...
                         (length string))
                       collect
                       (subseq string s-start s-end))))
            (values (list* :path '() :file forms) (coerce slices 'simple-vector))))))))

(defun write-atom (val stream)
  "Write the string representation of atomic VAL to STREAM."
  (match val
         ((type string) (format stream "~S" val))
         ((type character)
          (cond
         ...
         ((type keyword) (format stream ":~A" (string-downcase (symbol-name val))))
         ((type symbol) (write-string (string-downcase (symbol-name val)) stream))
         ((type number) (format stream "~A" val))
         (_ (format stream "~A" val))))

(defun format-atom (val)
  "Return the formatted string representation of VAL (compatibility wrapper)."
  (with-output-to-string (s) (write-atom val s)))
...
(defparameter *cl-indentify-dialects* '
...
(defun flat-collection-string (open blocks close)
  ...
  (with-output-to-string (s)
    ...
    (loop for
          ...
          do
          (let ((next-b (car rest)))
            (unless (member b '("#" "'") :test #'string=) (write-string " " s))))
...
(defun binding-form-multiline-p (children)
  ...
  (or (>= (length children) 4)
      (let ((binds (second children)))
        (and
          (member (get-node-tag binds) '(:paren :square))
...
(defun raw-form-text (node)
  ...
  (match node
         ((leaf _ val) (format-atom val))
         ((comment _ text) (trim-whitespace text))
         ((node _ tag children)
            ...
            (if (null children)
              ...
              (let*
                  ((blocks
                     (mapcar
                       (lambda (c)
                         (generic-form-text c (+ indent 2)))
                       children))
                   (flat (flat-collection-string open blocks close)))
                (if
                    (and (every #'single-line-p blocks) (<= (length flat) *max-line-column*))
                  flat
                  (with-output-to-string (s)
                    (write-string open s)
                    (write-string (first blocks) s)
  ...
  (with-input-from-string (in raw-text)
    (with-output-to-string (out)
      (handler-case (indentify:indentify in out)
        (error (e)
          (format *error-output*
                  "~&cl-indentify reindent failed (~A); using raw text.~%"
                  e)
...
(defun shift-indent-text (text base-indent)
  ...
  (if (zerop base-indent)
    ...
    (with-output-to-string (s)
      (let
          ((pad (make-indent-string base-indent)) (start 0))
        (loop for
              pos
              =
...
(defun clean-source-for-child
       (child clean-children clean-sources)
  "Return clean source string for CHILD if it exists unmodified in CLEAN-CHILDREN."
  (when (and clean-children clean-sources)
    (let
        ((pos (position child clean-children :test #'equal)))
      (when (and pos (< pos (length clean-sources))) (aref clean-sources pos)))))

(defun print-file-with-clean-sources
       (file-node clean-node
        clean-sources
        stream
        &optional
  ...
  (let*
      ((current-children (get-node-children file-node))
    ...
    (loop for
          ...
          do
          (let
              ((clean-str (clean-source-for-child c clean-children clean-sources)))
            (if clean-str
              (write-string clean-str stream)
              (progn (print-sexp c stream 0 :dialect dialect)
                     (when rest
                       (terpri stream)
                       (terpri stream))))))))
...
(defun toplevel-container-tag-p (tag)
  ...
  (or (member tag '(:file file :workspace workspace)) (supported-dialect-p tag)))

(defun print-toplevel-node
       (expr stream indent dialect)
  "Print top-level container sequence for EXPR."
  (match expr
         ((node _ tag children)
          (let
              ((d
                 (if (supported-dialect-p tag)
                   tag
                   dialect)))
...
(defun print-collection-expr (expr stream indent)
  ...
  (let ((tag (get-node-tag expr)))
    (cond
      ((collection-tag-p tag) (print-formatted-form expr stream indent))
      ((and (consp expr) (collection-tag-p (first expr)))
        (print-formatted-form ` (:path nil , (first expr) ,@ (rest expr)) stream indent))
      (t (print-formatted-form ` (:path nil :paren ,@expr) stream indent)))))

(defun print-leaf-expr (expr stream)
  "Serialize atomic or leaf node EXPR to STREAM."
  (match expr
         ((leaf _ val) (write-atom val stream))
         ((structural-editing-mcp.tree::comment _ text)
          (write-string (trim-whitespace text) stream))
         ((list :path _ val) (write-atom val stream))
         (_ (write-atom expr stream))))

(defun print-sexp
       (expr stream &optional (indent 0) &key (dialect *current-dialect*))
  ...
  (let ((*current-dialect* dialect))
    (cond
      ...
      ((or
         (member (get-node-tag expr)
                 '
                 (:paren :square :curly :set 'paren 'square 'curly 'set))
         (and (consp expr) (not (eq (first expr) :path))))
        (print-collection-expr expr stream indent))
      (t (print-leaf-expr expr stream)))))

(defun sexp-to-string
       (expr &key (indent 0) (dialect *current-dialect*))
  "Serialize an s-expression back into its string representation with proper formatting."
  (with-output-to-string (out) (print-sexp expr out indent :dialect dialect)))